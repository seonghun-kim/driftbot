...
| 조건 | 판정 |
|------|------|
| 승리 (아레나) | 모든 목표에 도달 (플레이어 원 ∩ 각 목표 원) |
| 승리 (복도) | 3개 게이트 해제 + 플레이어가 finishY 위로 이동 |
...
### 5.7 이동 업데이트 (매 tick)
```
FRICTION = 0.99995  // 사실상 무감쇠 (우주 공간)
...
| `FRICTION` | `0.99995` | 속도 감쇠 계수 (사실상 무감쇠) |
...
Y=0  ┌───────────────────────────────┐ (top)
     │          FINISH (Y=200)       │
Y~350│  ┌─ GATE 3 (3 items) ──────┐  │
     │  │  airlock(right)          │  │
     │  │  ╶╶ traversal bars ╶╶   │  │
Y~850│  ┌─ GATE 2 (2 items) ──────┐  │
     │  │  airlock(left)           │  │
     │  │  ╶╶ traversal bars ╶╶   │  │
Y~1350│ ┌─ GATE 1 (1 item)  ──────┐  │
     │  │  airlock(right)          │  │
     │  │  ╶╶ traversal bars ╶╶   │  │
Y=2000└───────────────────────────────┘ (bottom, player start)
```

| 파라미터 | 값 |
|---------|-----|
| 월드 크기 | 900 × 2000 |
| 복도 범위 | x=200~700 (폭 500) |
| 게이트 Y | 1350, 850, 350 |
| 트래버설 바 | 120px 돌출 × 25px 두께, 벽에서 교대 |
| 에어락 갭 | 120px (게이트 아래쪽) |
| 플레이어 시작 | (450, 1984), inventory 5 |
| finishY | 200 |

### 12.2 EVA 서브월드 시스템

에어락 통과 시 전체 월드를 교체하는 서브월드 전환 방식:

```
Corridor World               EVA World
┌─────────┐    airlock     ┌──────────┐
│ segments │ ────────────▶ │ 경계 벽만  │
│ debris   │    enter      │ EVA debris│
│ player   │               │ player    │
└─────────┘    ◀──────────  └──────────┘
    saved       return       loaded
```

- **진입**: 플레이어가 복도 벽의 에어락 갭을 통과 → 복도 상태 저장 → EVA 월드 로드
- **복귀**: EVA 경계 벽(returnEdge)에 벽 부착 + 30틱 체류 → 복도 상태 복원
- **상태 보존**: inventory, 수집한 아이템 수가 전환 간 유지됨
- **카메라**: 전환 시 에어락 방향으로 가로 슬라이드, CAMERA_LERP가 자연 애니메이션

### 12.3 게이트 상태 머신

```
LOCKED ──(items collected + 복도 복귀)──▶ UNLOCKED
                                          │
                              세그먼트 좌표 → (-1e5, -1e5)
                              체인 재빌드
                              currentGate++
...
- 게이트는 일반 벽 세그먼트로 구현, 해제 시 좌표를 (-1e5,-1e5)로 이동하여 비활성화
- 인덱스 유지 → wallSegIdx 참조 안전
- 새 Command 타입 없음 → 리플레이 호환성 100%

### 12.4 EVA 월드 정의

| EVA | 크기 | 아이템 수 | 에어락 | returnEdge |
|-----|------|----------|--------|------------|
| EVA 1 | 400×400 | 1 | right | left |
| EVA 2 | 400×400 | 2 | left | right |
| EVA 3 | 500×500 | 3 | left | left |
